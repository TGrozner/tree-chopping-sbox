# AGENTS.md — Tree Chopping (s&box)

Notes pour les futures sessions Codex qui bossent sur ce projet en autonomie. Ce projet est un proto **s&box** (moteur Source 2 + C#/.NET, par Facepunch). Voir `README.md` pour le pitch gameplay.
...
**Codex migration checkpoint (2026-05-22)** :
...
- **Stop hook** lance `tools\selftest.ps1` (1 seed, ~12s) si `Tree.cs` / `GameState.cs` / `SceneStarter.cs` / `BeaverController.cs` / `WoodItem.cs` / `ShopStation.cs` apparaissent dans `git status`. Échec → bloque le stop une fois ; appel de Stop suivant override (`stop_hook_active` guard). Étape 2 partiellement appliquée — seulement pour les fichiers du chop/wood path.
...
4. **Trigger Play en autonomie** (pour screenshots/vidéos sans demander à l'utilisateur) :
   ...
   - Pattern PowerShell complet dans `~/.Codex/projects/.../memory/sbox-screenshot-runtime.md`.
...
## Avec l'éditeur — MCP Codex Bridge

Installé 2026-05-19. Un MCP `sbox` (LouSputthole/Sbox-Codex v1.3.1) expose 99 tools qui drivent l'éditeur s&box vivant : `start_play`/`stop_play`/`is_playing`, `take_screenshot`, `get_scene_hierarchy`, `set_property`, `describe_type`/`get_method_signature` (reflection live), `invoke_button`, etc. Détails complets et catalogue dans [[sbox-Codex-bridge-mcp]].

**Prérequis runtime** :
1. `sbox-dev.exe` ouvert sur `tree_chopping.sbproj`.
2. **View → Codex Bridge** — le dock doit rester visible (le `[EditorEvent.Frame]` ne tick que dock-on-screen, sinon silent timeout).
...
**Paths** : repo MCP server configuré dans `.codex/config.toml`, addon bridge dans `Libraries/claudebridge/` (committable), IPC dans `%TEMP%\sbox-bridge-ipc\`.
...
| `Assets/scenes/main.scene` | Scène JSON minimale : `Sun` (DirectionalLight, overridden au runtime par SceneStarter.SetupLighting) + `Skybox` (SkyBox2D, warm orange tint) + `Fog` (GradientFog, copper-orange `0.92,0.62,0.42`) + `Ground` (plane initial — désactivé au boot, terrain heightmap prend le relais) + `Bootstrap` (SceneStarter, TreeCount=400, MinSpacing=180, BeaverSpawn=(-1000,0,80), SpawnPadRadius=180) + `Camera` (FieldOfView=72, Tonemapping HableFilmic + Bloom + ColorGrading). Tout le reste (Citizen player + axe, terrain procédural, mountain borders, forêt biome-biased, ShopStation, WoodHud, GameState, AutoPlay) est spawné au runtime par `SceneStarter`. |
...
| `.codex/hooks.json` | Hooks Codex — voir section "Harness automation" plus haut. PostToolUse = build auto, Stop = selftest auto. |
...
Prestige loop (Cookie-Clicker / AdVenture-Capitalist pattern) :
  ...
  └─ ShopStation.Prestige : replant + feedback prestige
...
→ Déplacé dans `Code/AGENTS.md` (chargé seulement quand tu touches `Code/**/*.cs`). Y a : Component lifecycle, attributs Property/RequireComponent/Sync, `Components.Get` patterns, spawn par code, `Scene.Trace`, Input, HUD immediate-mode, Z-up / inches / WorldScale / `Model.Cube` tint gotcha, hotload pièges, MSTest setup, et où chercher la doc à jour (context7, XML locaux, base addon).

## Workflow Codex — recommandé pour ce repo

1. **Pas de commit sans demande explicite.** Lire AGENTS.md global comportement.
