...
- **`SelfTest.cs`** : phase contract dérivé par `tools/selftest.ps1` depuis l'enum `Phase`; couvre swing réel, spawn distribution, stump/respawn, split, pickup/deposit, cascade, too-hard, stats, prestige. GameState.Save short-circuits when active (no clobber)
